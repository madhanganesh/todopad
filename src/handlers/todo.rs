...
use super::{
    BaseTemplate, CurrentUser, HtmlTemplate, get_todos_and_show_date, spawn_get_tags_and_save,
};
use crate::repo;
...
pub async fn create_todo(
    ...
    match repo::todo::create_todo(&pool, user.user_id, &form.title, &due).await {
        ...
        Err(_) => Html("<p>Error creating todo</p>".to_string()).into_response(),
    }
}
...
pub async fn get_todos(
    ...
    let filter: &str = query_params
        .get("filter")
        .map(String::as_str)
        .unwrap_or("pending");

    let (todos, show_date) = get_todos_and_show_date(filter, &pool, user.user_id, &timezone).await;
...
async fn get_todo_response(
    ...
    let tags = repo::todo::get_tags_for_todo(pool, user_id, todo_id)
        .await
        .unwrap_or(vec![]);
...
pub async fn edit_todo(
    ...
    get_todo_response(headers, &pool, user.user_id, todo_id, None).await
}
...
pub async fn get_todos_trends(
    ...
    let (todos, _) = get_todos_and_show_date(&filter, &pool, user.user_id, &timezone).await;
...
pub async fn update_effort(
    ...
    let mut todo = repo::todo::get_todo(&pool, user.user_id, todo_id)
        .await
        .unwrap();
    ...
    let response_html = format!(r#"<span id="effort-{}">{:.1}</span>"#, todo_id, new_effort);
